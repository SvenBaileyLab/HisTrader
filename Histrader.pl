...
sub getMaxNucValley {
    ...
    return ("NA-NA", "NA-NA") if !@nucs;

    my %maxNuc;
    my %minNFRSignal;
    my %nfrDifferentials;

    # Calculate maximum signal for nucleosomes
    for (my $j = 0; $j <= $#nucs; $j++) {
        my @nucSS = split(/-/, $nucs[$j]);
        my @tmpNucSignal;

        for (my $i = 0; $i <= $#chipStep; $i++) {
            my @oriSS = split(/-/, $chipStep[$i]);

            if ($oriSS[0] >= $nucSS[0] && $oriSS[1] <= $nucSS[1]) {
                push(@tmpNucSignal, $chipSig[$i]);
            }
        }

        $maxNuc{$nucs[$j]} = my_max(@tmpNucSignal) if @tmpNucSignal;
    }

    # Calculate the minimum signal for NFRs
    for (my $k = 0; $k <= $#open; $k++) {
        my @nfrSS = split(/-/, $open[$k]);
        my @tmpNFRSignal;

        for (my $i = 0; $i <= $#chipStep; $i++) {
            my @oriSS = split(/-/, $chipStep[$i]);

            if ($oriSS[0] >= $nfrSS[0] && $oriSS[1] <= $nfrSS[1]) {
                push(@tmpNFRSignal, $chipSig[$i]);
            }
        }

        $minNFRSignal{$open[$k]} = my_min(@tmpNFRSignal) if @tmpNFRSignal;
    }

    if ($useDifferential eq "TRUE") {
        # Calculate the differential-based method
        my $maxDifferential = -1;
        my $bestNFR = "NA-NA";

        for (my $k = 0; $k < $#nucs; $k++) {
            if ($k < $#nucs) {
                my $leftNuc = $nucs[$k];
                my $rightNuc = $nucs[$k + 1];

                if (exists $maxNuc{$leftNuc} && exists $maxNuc{$rightNuc}) {
                    my $nucSum = $maxNuc{$leftNuc} + $maxNuc{$rightNuc};

                    if ($k < $#open) {
                        my $currentNFR = $open[$k + 1];
                        my $nfrSignal = $minNFRSignal{$currentNFR} // 1e9;

                        my $differential = $nucSum - $nfrSignal;
        ...
        # Return all nucleosome signals and the NFR with the largest differential
        my @allNucSignals = map { $maxNuc{$_} // "NA" } @nucs;
        return (\@allNucSignals, $bestNFR);

    } else {
        # Return nucleosome with the smallest NFR signal
        my $maxNucSig;
        my $maxValue = -1;

        while ((my $nuc, my $sig) = each %maxNuc) {
            if ($sig > $maxValue) {
                $maxValue = $sig;
                $maxNucSig = $nuc;
            }
        ...
        for (my $k = 0; $k <= $#nucs; $k++) {
            if ($nucs[$k] eq $maxNucSig) {
                if ($k > 0) {
                    $leftSignal = $minNFRSignal{$open[$k - 1]} // 1e9;
                }
                if ($k < $#nucs) {
                    $rightSignal = $minNFRSignal{$open[$k]} // 1e9;
                }

                if (defined $leftSignal && defined $rightSignal) {
                    $maxDHS = ($leftSignal < $rightSignal) ? $open[$k - 1] : $open[$k];
                } elsif (defined $leftSignal) {
                    $maxDHS = $open[$k - 1];
                } elsif (defined $rightSignal) {
                    $maxDHS = $open[$k];
                }
                last;
            }
                 ...
                 if(@{$newSignal} && @{$newStep}){
                     ...
                     if($method eq "DIFF"){
                         if(@dhs){
                            ...
                            if($randValley eq "TRUE"){
                              if(@dhsF != 0){
                                my $randDHS = splice(@dhsF, int rand @dhsF, 1);
                                @dhsF=(); 
                                push(@dhsF,$randDHS);
                              }
                            }       
                     ...
                     if($method eq "BOTH"){
                         if(@overlap){
                           ...
                           if($randValley eq "TRUE"){
                              if(@overlapF != 0){
                                 my $randDHS = splice(@overlapF, int rand @overlapF, 1);
                                 @overlapF=();
                                 push(@overlapF, $randDHS);
                              }
                           }       
                           printBed($chr,$probe,\@overlapF,$db);
                           printBed($chr,$probe,\@updateMerge,$nb);
                           if(defined $genome){
                              printFasta($chr,$probe,\@overlapF,\%genome,$df,$trim,$trimSize);
                              printFasta($chr,$probe,\@updateMerge,\%genome,$nf,$trim,$trimSize);
                           } 
                         }
                     }  
                     ...
                     if($method eq "MA"){
                        if(@free){
                          my @freeF=filterBed(\@free,$filter);
                            ...
                            if($randValley eq "TRUE"){
                              if(@freeF != 0){
                                 my $randDHS = splice(@freeF, int rand @freeF, 1);
                                 @freeF=();
                                 push(@freeF, $randDHS);
                              }
                            }       
...
sub printBed{
   ...
   # Check if the array reference is empty
   return unless @{$c};
...
sub printHeader{
    ...
    print "##                                                                                                    ##\n";
    print "##                                                                                                    ##\n";
    print "##                  Written by Eftyhios Kirbizakis, Yifei Yan, and Swneke D. Bailey                   ##\n";
...
GetOptions( "bedGraph:s"   => \$chip,   # --bedGraph ... Histone ChIP-seq bedgraph
	    ...
	    "genome:s"     => \$genome,   # --genome ... Genome fasta (Optional)
            ...
            "maxValley:s"  => \$maxValley, #only print valley at max peak
            "useDifferential:s"  => \$useDifferential, #only print valley at max peak
            "randValley:s" => \$randValley, #only print one random valley
...
if($maxValley eq "TRUE" && $randValley eq "TRUE"){
   print "\nERROR:  --maxValley and --randValley cannot be used together.\n";
   exit();
}
